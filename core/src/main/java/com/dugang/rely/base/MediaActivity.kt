...
import com.dugang.rely.common.extension.createFolder
...
abstract class MediaActivity : BaseActivity() {
    ...
    open fun pickPictureCallBack(filePath: String) {
        Log.d("MediaActivity", "pickPictureCallBack: $filePath")
    ...
    open fun takeVideoCallBack(filePath: String) {
        Log.d("media", "takeVideoCallBack: $filePath")
    }
    ...
    private fun getMediaPath(uri: Uri, isVideo: Boolean = false): String? {
        val projection = if (isVideo) arrayOf(MediaStore.Video.VideoColumns.DATA) else arrayOf(MediaStore.Images.ImageColumns.DATA)
